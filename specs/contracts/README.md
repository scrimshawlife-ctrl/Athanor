...
Each receipt requires schema_version, run_id, workflow_id, job_type, status, started_at, finished_at, engine, config_hash, inputs, outputs, counts, errors and epistemic. References carry identity, sha256 and optional authorized location. job_type covers harvest, normalize, gold_settle, freeze, sanitize_export, tooling_check, train, eval, publish, handoff and registry_review. WF-004 uses gold_settle, matching the existing gold-settle protocol; the earlier advisory enum spelling settlement is not a v1 alias. Historical receipt bytes remain unchanged. A receipt for PARTIAL/FAILED/CANCELLED must enumerate errors or cancellation reason; SUCCEEDED must identify required outputs.
...
Executed E1/E2/E3 comparison results require numeric value and control_value for F1, exact-match and recall@5 comparisons; zero is a valid measurement, while numeric strings and named baselines are invalid. NOT_COMPUTABLE results and non-comparison gates may retain null controls. Every executed PASS/FAIL requires a nonblank reviewer. A null encoder model_ref forces outcome=unavailable, never an out_of_domain classification. The v1 encoder retains the optional v0 disposition enum (ok, historical_only, refuse_or_historical_only) at the encoder-block location documented by the dual-use addendum; it does not grant authority or override efficacy=null.
