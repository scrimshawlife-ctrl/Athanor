...
Review regression check: `python specs/contracts/verify_review.py` (requires jsonschema). It tests positive and negative cases, both PASS and FAIL paths, nonblank reviewers for executed results, and WF-003 error-envelope consistency. It needs no Git history and ignores unrelated tracked/untracked changes. One-off patch-scope and whitespace audits run separately at review time; this checker does not certify real gate outcomes or patch scope.
